...
export function buildWorkflowAgentPrompt({ workflowId, objective }: WorkflowAgentPromptInput): string {
  ...
  return [
    ...
    "- For every LLM-to-script handoff, make the LLM prompt populate the exact outputFields key consumed by the script. Downstream bindings read outputs[upstreamOutputKey], never the LLM summary.",
    '- Example LLM-to-script binding: an LLM node with id="research" and outputFields=[{"key":"answer","required":true}] connects directly to a script parameter {"key":"answer","source":"upstream","upstreamNodeId":"research","upstreamOutputKey":"answer","location":"body","valueType":"string","required":true}.',
