...
export function buildWorkflowAgentPrompt({ objective }: WorkflowAgentPromptInput): string {
  ...
  return [
    ...
    "- Classify each executable LLM node as one-shot or interactive before creating the graph.",
    "- Use one-shot only when every required input is already available and the node can finish without requesting user input.",
    "- Use interactive whenever the node may need clarification, supplemental information, iterative choices, or user confirmation during execution, even if the expected question is simple.",
    "- Do not rely on runtime one-shot-to-interactive upgrade as the normal planning strategy; it is only a fallback for misclassification.",
