...
export function createNormalizedSchema(db: SqliteSchemaDatabase): void {
  db.exec(`
    ...
    create table if not exists evaluation_evaluators (id text primary key, name text not null, kind text not null, prompt text, rubric_json text, agent_id text, runtime_id text, threshold real not null, enabled integer not null, created_at integer not null, updated_at integer not null);
  ...
  ensureColumn(db, "evaluation_evaluators", "rubric_json", "text");
