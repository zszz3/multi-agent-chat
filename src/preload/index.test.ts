...
describe("preload skill API", () => {
  test("exposes local install controls without runtime skill translation", async () => {
    ...
    expect(electronState.exposedApi).toHaveProperty("listSlashCompletions");
  ...
  test("keeps runtime session typing stable across the preload snapshot surface", () => {
    ...
    expectTypeOf<Awaited<ReturnType<MultiAgentChatApi["listSlashCompletions"]>>>().toEqualTypeOf<
      import("../shared/types").SlashCompletionGroup[]
    >();
